package com.opencdk.common.download;
...
import com.opencdk.common.util.http.FileAsyncHttpResponseHandler;
import com.opencdk.core.cache.XCacheFactory;
import com.opencdk.util.CommonUtils;
...
public class DownloadService extends Service
{
	...
	public static final String TAG = "CDK.DownloadService";
			...
			final String destination)
	{
		...
		{
			// 已经存在此任务
			Toast.makeText(getApplicationContext(), R.string.cdk_download_url_repeat, Toast.LENGTH_SHORT).show();
			return;
		}
		...
		{
			// 下载地址为空, 即无效的下载地址
			Toast.makeText(getApplicationContext(), R.string.cdk_download_url_invalid, Toast.LENGTH_SHORT).show();
	...
	private void completeDownload(Context context, Request request, File tempFile)
	{
		...
		{
			// 一般情况下, 不可能发生的情况
			Toast.makeText(context, R.string.cdk_download_file_not_found, Toast.LENGTH_LONG).show();
	...
	private File createNewTempFile(String fileName, String downloadUrl, String destination)
	{
		...
		{
			// 保存地址为空, 使用默认的地址
			file = new File(XCacheFactory.getWo2bDownloadDir(), tempFileName);
		}
		else
		{

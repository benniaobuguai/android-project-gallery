...
public class UserActivity extends BaseFragmentActivity
{
	...
	{
		if (msg.what == CDKEvent.USER_LOGIN_CMD)
		{
			onLoginCmd();
		}
		else if (msg.what == CDKEvent.USER_LOGIN_OK)
		{
			onLoginOK((User) msg.obj);
		}
		else if (msg.what == CDKEvent.USER_LOGIN_FAIL)
		{
			onLoginFail(msg.arg1, msg.obj + "");
		}
		else if (msg.what == CDKEvent.USER_LOGOUT_CMD)
		{
			onLogoutCmd();
		}
		else if (msg.what == CDKEvent.USER_LOGOUT_OK)
		{
			onLogout();
		}
		else if (msg.what == CDKEvent.USER_RESET_PWD_CMD)
		{
			resetPwdCmd();
		}
		else if (msg.what == CDKEvent.USER_RESET_PWD_OK)
		{
			onResetPwdOK();
		}
	}
